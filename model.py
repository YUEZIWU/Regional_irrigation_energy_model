...
def create_model(args):
    ...
    # solar_cap_cost    = args.num_year * annualization_solar * float(args.solar_cost_kw)
    ...
    print('irrigation area')
    print(irrigation_area)

    # set up regional model
    for i in range(args.num_regions):

        # Initialize capacity variables
        # solar_cap       = m.addVar(obj=solar_cap_cost, name='solar_cap_region_{}'.format(i + 1))
        ...
        # replace irrigation_area[i] by 1000, 2000, 3000, 5000, 10000, 20000
        # irrigation_area[i] = args.single_node_area
        ...
        for d in list(range(args.first_season_start, args.first_season_end - args.water_account_days + 2)) + \
            ...
            rain_kg_in_period = 0
                # sum(rain_rate_daily_mm_m2[k] * irrigation_area[i]
